...
class ReCoLoc(nn.Module):
    """
    implementation of the hierarchical scene coordinate network
    """
    def __init__(self, training=True, dataset='7S',use_reco=False, output_dim=256):
        super(ReCoLoc, self).__init__()
        ...
        if use_reco:
            self.convrep2 = conv(conv_planes_c2[0] + conv_planes_c2[2],
                        conv_planes_c2[3], kernel_size=1, stride=1)
            self.convrep3 = conv(conv_planes_c2[3], conv_planes_c2[4],
                        kernel_size=1, stride=1)
            self.convoutrep2 = conv_(conv_planes_c2[4], output_dim)
        ...
        if use_reco:
            self.convrep5 = conv(conv_planes_c1[3] + conv_planes_c2[0],
                        conv_planes_c1[4], kernel_size=1, stride=1)
            self.convrep6 = conv(conv_planes_c1[4], conv_planes_c1[5],
                        kernel_size=1, stride=1)
            self.convoutrep1 = conv_(conv_planes_c1[5], output_dim)
    ...
    def forward(self, x, lbl_1=None, lbl_2=None):
        ...
        out_f = self.convc5(torch.cat((out, out_convc1), 1))
        out_feat_1 = self.convc6(out_f)
        out_lbl_1 = self.convoutc1(out_feat_1)

        if self.use_reco:
            out_r = self.convrep5(torch.cat((out, out_convc1), 1))
            out_r = self.convrep6(out_r)
            out_rep_1 = self.convoutrep1(out_r)
        ...
        out_f = self.cond(self.convc2(torch.cat((out, out_convc1), 1)),
                    out_gconv1_gamma_1,out_gconv1_beta_1)
        out_feat_2 = self.cond(self.convc3(out_f), out_gconv1_gamma_2,
                    out_gconv1_beta_2)
        out_lbl_2 = self.convoutc2(out_feat_2)
        if self.use_reco:
            out_r = self.cond(self.convrep2(torch.cat((out, out_convc1), 1)),
                    out_gconv1_gamma_1,out_gconv1_beta_1)
            out_r = self.cond(self.convrep3(out_r), out_gconv1_gamma_2,
                    out_gconv1_beta_2)
            out_rep_2 = self.convoutrep2(out_r)
        ...
        if self.use_reco:
            return out_coord, out_lbl_2, out_lbl_1, out_rep_2, out_rep_1
        else:
            return out_coord, out_lbl_2, out_lbl_1, out_feat_2,out_feat_1
